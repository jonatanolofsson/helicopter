...
namespace sys {
    namespace motioncontrol {
        ...
        void MotionControl<Algorithm, MotionModel, SystemStateMessage, Reference, ControlMessage>
        ::updateControl(const SystemStateVector systemState, const Reference reference) {
            ...
            auto controlState = Algorithm::States::template translate<SystemStates>(systemState);
            static const int linState = Algorithm::linState;

            auto A = math::template differentiate<MotionModel, typename Algorithm::States, SystemStates>(systemState);
            F.template block<MotionModel::nofStates, MotionModel::nofStates>(0,0) = A;
            F.template block<MotionModel::nofStates, 1>(0, linState) =
                    MotionModel::template predict<SystemStates>(systemState)
                    - A*controlState;
            ...
            B.template block<MotionModel::nofStates, Algorithm::nofControls>(0,0) =
                    math::template differentiate<MotionModel, typename Algorithm::Controls, SystemStates>(systemState);
